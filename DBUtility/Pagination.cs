...
namespace DBUtility
{
    ...
    public class Pagination
    {
        ...
        #region 方法
        /// <summary>
        /// 匹配当前页次、查询条件、排序语句等参数,获取数据表(TableName不能为子查询)
        /// </summary>
        /// <returns></returns>
        ...
        /// <summary>
        /// 匹配当前页次、查询条件、排序语句等参数,获取数据表(tableName为子查询)
        /// </summary>
        /// <returns></returns>
        public DataTable GetDataTable_Join()
        {
            SqlParameter[] parms = new SqlParameter[]
            { 
                new SqlParameter("@SqlWhere", SqlDbType.VarChar,8000),
            ...
            using (SqlConnection conn = new SqlConnection(SqlHelper.ConnectionStringLocalTransaction))
            {
                dt = SqlHelper.ExecuteDataTable(conn, CommandType.StoredProcedure, "Pagination_Join", parms);
                RecordCount = int.Parse((parms[7].Value.ToString()));
                conn.Close();
                conn.Dispose();
            }
